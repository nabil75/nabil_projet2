...
def get_book_elements(url):
    ...
    descripteurs.append(
        (bs.find("title").text).strip().replace(" | Books to Scrape - Sandbox", "")
    )
    ...
    descripteurs.append(
        str(bs.find("ul", {"class": "breadcrumb"}).find_all("li")[2].find("a").text)
    )
    ...
    path_img_file = (
        os.getcwd()
        + "/"
        + str(bs.find("table").find_all("tr")[0].find("td").text)
        + ".jpg"
    )
...
def get_category_elements(url, category_all_descripteurs):
    ...
    if li_next is not None:
        ...
        category_all_descripteurs = get_category_elements(
            next_page, category_all_descripteurs
        )
...
def get_all_books_elements(url, parent_dir):
    ...
    for category in list_category:
        ...
        category_descripteurs = get_category_elements(
            category_url, category_descripteurs
        )
...
print(
    str(now_begin.hour) + " h",
    str(now_begin.minute) + " min",
    str(now_begin.second) + " s",
)
...
print(
    str(now_end.hour) + " h", str(now_end.minute) + " min", str(now_end.second) + " s"
)
